    ...
    <div className="bg-panel-header-background h-20 px-4 flex items-center gap-6 relative">
      <>
        ...
        <div className="w-full rounded-lg h-10 flex items-center">
          <input type="text" placeholder="Type a message" className="bg-input-background text-sm focus:outline-none text-white h-10 rounded-lg px-5 py-4 w-full" onChange={(e) => setMessage(e.target.value)} value={message} onKeyDown={handleKeyDown} />
        </div>
        <div className="flex w-10 items-center justify-center">
          <button className="bg-primary rounded-full w-10 h-10 flex items-center justify-center">
            <MdSend title="Send Message" className="text-panel-header-icon text-xl cursor-pointer" onClick={sendMessage} />
