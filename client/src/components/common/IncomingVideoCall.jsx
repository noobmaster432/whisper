...
function IncomingVideoCall() {
  ...
  return (
    <div className="h-24 w-80 fixed bottom-8 mb-0 right-6 z-50 rounded-md flex gap-5 items-center justify-start p-4 bg-conversation-panel-background text-white drop-shadow-2xl border-icon-green border-2 py-14">
      <div>
      ...
      <div>
        ...
        <div className="text-xs animate-pulse">Incoming Video Call</div>
        <div className="gap-2 mt-2 flex">
          <button className="bg-red-500 py-1 px-3 text-sm rounded-full" onClick={rejectCall}>
            Reject
          </button>
          <button className="bg-green-500 py-1 px-3 text-sm rounded-full" onClick={acceptCall}>
            Accept
          </button>
        </div>
      </div>
